...
export default function LibraryPageContent({
  ...
  // ✅ Danh sách endpoint thử theo TAB
  const ENDPOINTS = useMemo(
    () => ({
      ...
      history: ["/history/me", "/me/history", "/history"],
      uploads: ["/me/uploads", "/songs/me", "/me/songs", "/uploads"],
    }),
    []
  );

  // ✅ SỬA 1: ưu tiên đúng chuẩn API: POST /playlists
  // fallback: /playlists/me (nếu backend bạn có)
  ...
  const createPlaylistRequest = async (name) => {
    ...
    for (const ep of CREATE_PLAYLIST_ENDPOINTS) {
      try {
        ...
        // ✅ SỬA 2: 405 cũng thử tiếp endpoint còn lại
